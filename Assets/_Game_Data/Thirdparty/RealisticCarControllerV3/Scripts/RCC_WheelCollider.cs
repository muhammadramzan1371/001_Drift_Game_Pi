...
public class RCC_WheelCollider : MonoBehaviour
{
	...
	{
		get { return RCC_GroundMaterials.Instance; }
	}

	private RCC_GroundMaterials.GroundMaterialFrictions[] physicsFrictions
	{
		get { return RCC_GroundMaterials.Instance.frictions; }
	}
	...
	void Awake()
	{
		...
		switch (RCC_Settings.Instance.behaviorType)
		{
			...
			case RCC_Settings.BehaviorType.Drift:
				...
				{
					Debug.LogError("Current behavior mode is ''Drift'', but your vehicle named " + carController.name +
		...
		if (RCC_Settings.Instance.useSharedAudioSources)
		{
			if (!carController.transform.Find("All Audio Sources/Skid Sound AudioSource"))
				audioSource = RCC_CreateAudioSource.NewAudioSource(carController.gameObject, "Skid Sound AudioSource",
					5, 50, 0, audioClip, true, true, false);
			else
		...
		{
			audioSource = RCC_CreateAudioSource.NewAudioSource(carController.gameObject, "Skid Sound AudioSource", 5,
				50, 0, audioClip, true, true, false);
	...
	WheelFrictionCurve SetFrictionCurves(WheelFrictionCurve curve, float extremumSlip, float extremumValue,
	...
	void FixedUpdate()
	{
		...
		wheelRPMToSpeed =
			(((wheelCollider.rpm * wheelCollider.radius) / 2.8f) * Mathf.Lerp(1f, .75f, hit.forwardSlip)) *
			rigid.transform.lossyScale.y;
		camber = this == carController.FrontLeftWheelCollider || this == carController.FrontRightWheelCollider
			? carController.frontCamber
	...
	public void WheelAlign()
	{
		...
		if (Physics.Raycast(ColliderCenterPoint, -wheelCollider.transform.up, out hit,
			    (wheelCollider.suspensionDistance + wheelCollider.radius) * transform.localScale.y) &&
		    !hit.transform.IsChildOf(carController.transform) && !hit.collider.isTrigger)
		{
			wheelModel.transform.position =
				hit.point + (wheelCollider.transform.up * wheelCollider.radius) * transform.localScale.y;
			float extension =
				(-wheelCollider.transform.InverseTransformPoint(CorrespondingGroundHit.point).y -
				 wheelCollider.radius) / wheelCollider.suspensionDistance;
			Debug.DrawLine(CorrespondingGroundHit.point,
				CorrespondingGroundHit.point + wheelCollider.transform.up * (CorrespondingGroundHit.force / rigid.mass),
				extension <= 0.0 ? Color.magenta : Color.white);
			Debug.DrawLine(CorrespondingGroundHit.point,
				CorrespondingGroundHit.point -
				wheelCollider.transform.forward * CorrespondingGroundHit.forwardSlip * 2f, Color.green);
			Debug.DrawLine(CorrespondingGroundHit.point,
				CorrespondingGroundHit.point - wheelCollider.transform.right * CorrespondingGroundHit.sidewaysSlip * 2f,
				Color.red);
		}
		else
		{
			wheelModel.transform.position = Vector3.Lerp(wheelModel.transform.position,
				ColliderCenterPoint - (wheelCollider.transform.up * wheelCollider.suspensionDistance) *
	...
	public void WheelCamber()
	{
		...
		if (wheelCollider.transform.localPosition.x < 0)
			wheelLocalEuler = new Vector3(wheelCollider.transform.localEulerAngles.x,
	...
	void SkidMarks()
	{
		...
		if (skidmarks)
		{
			...
			if (wheelSlipAmountSideways > startSlipValue || wheelSlipAmountForward > startSlipValue * 2f)
			{
				...
				{
					lastSkidmark = skidmarks.AddSkidMark(skidPoint, GroundHit.normal,
						(wheelSlipAmountSideways / 2f) + (wheelSlipAmountForward / 2f), lastSkidmark);
					wheelTemparature += ((wheelSlipAmountSideways / 2f) + (wheelSlipAmountForward / 2f)) /
					                    ((Time.fixedDeltaTime * 100f) * Mathf.Lerp(1f, 5f, wheelTemparature / 150f));
				}
				else
				{
					lastSkidmark = -1;
	...
	void Frictions()
	{
		...
		for (int i = 0; i < physicsFrictions.Length; i++)
		{
			...
			if (GroundHit.point != Vector3.zero &&
			    GroundHit.collider.sharedMaterial == physicsFrictions[i].groundMaterial)
			{
				...
				sidewaysFrictionCurve.stiffness =
					(physicsFrictions[i].sidewaysStiffness * tractionHelpedSidewaysStiffness);
				...
				if (wheelSlipAmountSideways > physicsFrictions[i].slip ||
				    wheelSlipAmountForward > physicsFrictions[i].slip)
				{
		...
		if (!contacted && physicsMaterials.useTerrainSplatMapForGroundFrictions)
		{
			...
			for (int k = 0; k < physicsMaterials.terrainSplatMapIndex.Length; k++)
			{
				...
				if (GroundHit.point != Vector3.zero &&
				    GroundHit.collider.sharedMaterial == physicsMaterials.terrainPhysicMaterial)
				{

					if (TerrainSurface.GetTextureMix(transform.position) != null &&
					    TerrainSurface.GetTextureMix(transform.position)[k] > .5f)
					{

						contacted = true;

						forwardFrictionCurve.stiffness = physicsFrictions[physicsMaterials.terrainSplatMapIndex[k]]
							.forwardStiffness;
						sidewaysFrictionCurve.stiffness =
							(physicsFrictions[physicsMaterials.terrainSplatMapIndex[k]].sidewaysStiffness *
						...
						if (wheelSlipAmountSideways > physicsFrictions[physicsMaterials.terrainSplatMapIndex[k]].slip ||
						    wheelSlipAmountForward > physicsFrictions[physicsMaterials.terrainSplatMapIndex[k]].slip)
						{
							emission.enabled = true;
						}
						else
	...
	void Audio()
	{
		...
		if (totalSlip > startSlipValue)
		{
			...
			if (rigid.velocity.magnitude > 1f)
			{
				audioSource.volume = Mathf.Lerp(audioSource.volume, Mathf.Lerp(0f, 0.3f, totalSlip - startSlipValue),
					Time.deltaTime * 5f);
