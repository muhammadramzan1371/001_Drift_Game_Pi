...
using System.Collections;
 ...
 public class RCC_Light : MonoBehaviour
 {
	 ...
	 {
		 get { return RCC_Settings.Instance.indicatorClip; }
	 }
	 ...
	 void Start()
	 {
		 ...
		 if (RCC_Settings.Instance.useLightProjectorForLightingEffect)
		 {
			 ...
			 if (projector == null)
			 {
				 projector = ((GameObject)Instantiate(RCC_Settings.Instance.projector, transform.position,
					 transform.rotation)).GetComponent<Projector>();
			 ...
			 if (lightType != LightType.HeadLight)
				 projector.transform.localRotation =
					 Quaternion.Euler(20f, transform.localPosition.z > 0f ? 0f : 180f, 0f);
		 ...
		 if (lightType == LightType.Indicator)
		 {
			 ...
			 if (!carController.transform.Find("All Audio Sources/Indicator Sound AudioSource"))
				 indicatorSound = RCC_CreateAudioSource.NewAudioSource(carController.gameObject,
					 "Indicator Sound AudioSource", 3, 10, 1, indicatorClip, false, false, false);
			 else
